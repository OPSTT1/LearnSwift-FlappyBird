...
class GameScene: SKScene , SKPhysicsContactDelegate{
    ...
    var 上一次更新代码: NSTimeInterval = 0
    var dt: NSTimeInterval = 0
    //  创建音效
    let 叮的音效 = SKAction.playSoundFileNamed("ding.wav", waitForCompletion: false)
    let 拍打的音效 = SKAction.playSoundFileNamed("flapping.wav", waitForCompletion: true)
    ...
    func 设置主角(){
        ...
        CGPathMoveToPoint(path, nil, 18 - offsetX, 27 - offsetY)
        ...
        CGPathAddLineToPoint(path, nil, 34 - offsetX, 4 - offsetY)
        CGPathAddLineToPoint(path, nil, 37 - offsetX, 8 - offsetY)
    ...
    func 设置得分标签(){
        ...
        得分标签.fontColor = SKColor(red: 101.0/225.0, green: 71.0/225.0, blue: 73.0/225.0, alpha: 1.0)
    ...
    func 设置记分板(){
        ...
        let 当前记分板 = SKLabelNode(fontNamed: k字体名字)
        当前记分板.fontColor = SKColor(red: 101.0/225.0, green: 71.0/225.0, blue: 73.0/225.0, alpha: 1.0)
        当前记分板.position = CGPoint(x: -记分板.size.width/4, y: -记分板.size.height/2+20.0)
        当前记分板.text = "\(当前分数)"
        当前记分板.zPosition = 图层.UI.rawValue
        记分板.addChild(当前记分板)
        
        let 最高分标签 = SKLabelNode(fontNamed: k字体名字)
        最高分标签.fontColor = SKColor(red: 101.0/225.0, green: 71.0/225.0, blue: 73.0/225.0, alpha: 1.0)
        最高分标签.position = CGPoint(x: 记分板.size.width/4, y: -记分板.size.height/2+20.0)
        最高分标签.text = "\(最高分())"
        ...
        let 向上移动动画 = SKAction.moveTo(CGPoint(x: size.width/2, y: size.height/2), duration: k动画延迟)
        向上移动动画.timingMode = .EaseInEaseOut
    ...
    func 创建障碍物(图片名:String) -> SKSpriteNode{
        ...
        CGPathMoveToPoint(path, nil, 3 - offsetX, 310 - offsetY)
        CGPathAddLineToPoint(path, nil, 49 - offsetX, 312 - offsetY)
        CGPathAddLineToPoint(path, nil, 48 - offsetX, 1 - offsetY)
        CGPathAddLineToPoint(path, nil, 3 - offsetX, 2 - offsetY)
        CGPathCloseSubpath(path)
    ...
    func 生成障碍(){
        ...
        顶部障碍.zPosition = CGFloat(180).degreesToRadians()
        顶部障碍.position = CGPoint(x: 起始X坐标, y: 底部障碍.position.y + 底部障碍.size.height/2 + 顶部障碍.size.height/2+主角.size.height * k缺口常数)
        顶部障碍.name = "顶部障碍"
        世界单位.addChild(顶部障碍)
        
        let X轴移动距离:CGFloat = -(size.width + 底部障碍.size.width)
    ...
    func 无限重生障碍(){
        ...
        let 总动作队列 = SKAction.sequence([首次延迟,无限重生])
        runAction(总动作队列,withKey: "重生")
    }
    ...
    func 更新主角(){
        ...
        var ll = CGPoint(x: 1, y: 1)
        ll = ll + CGPointMake(2, 2)
    ...
    func 更新得分(){
        世界单位.enumerateChildNodesWithName("顶部障碍", usingBlock: { 匹配单位, _ in
            if let 障碍物啊 = 匹配单位 as? SKSpriteNode{
                if let 已通过 = 障碍物啊.userData?["已通过"] as? NSNumber{
                    if 已通过.boolValue{
                        return
                    }
                }
                
                if self.主角.position.x > 障碍物啊.position.x + 障碍物啊.size.width / 2 {
                    print("-- \(self.当前分数)")
                    self.当前分数 += 1
                    self.得分标签.text = "\(self.当前分数)"
                    self.runAction(self.得分的音效)
                    障碍物啊.userData?["已通过"] = NSNumber(bool: true)
                }
            }
        })
    }
    ...
    func 切换到跌落状态() {
        当前的游戏状态 = .跌落
        runAction(SKAction.sequence([摔倒的音效,SKAction.waitForDuration(0.1),下落的音效]))
        主角.removeAllActions()
        停止重生障碍()
    ...
    func 切换到新游戏() {
        ...
        let 切换特效 = SKTransition.fadeWithColor(SKColor.greenColor(), duration: 0.05)
